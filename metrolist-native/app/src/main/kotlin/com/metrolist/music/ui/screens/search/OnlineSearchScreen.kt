...
fun OnlineSearchScreen(
    ...
    LazyColumn(
        ...
        val hasLocalResults = viewState.localSongs.isNotEmpty() || viewState.localArtists.isNotEmpty() || viewState.localAlbums.isNotEmpty()
        if (hasLocalResults) {
            ...
            items(viewState.localSongs, key = { "local_song_${it.id}" }) { item ->
                SongListItem(
                    song = item,
                            ...
                            .combinedClickable(
                                onClick = {
                                    if (item.id == mediaMetadata?.id) {
                                        ...
                                        val songs = viewState.localSongs.map { it.toMediaItem() }
                                        playerConnection.playQueue(
                                            ListQueue(
            ...
            items(viewState.localArtists, key = { "local_artist_${it.id}" }) { item ->
                ArtistListItem(
                    artist = item,
                    modifier =
                        Modifier
            ...
            items(viewState.localAlbums, key = { "local_album_${it.id}" }) { item ->
                AlbumListItem(
                    album = item,
            ...
            item(key = "local_library_divider") {
                HorizontalDivider(
...
fun SuggestionItem(
    ...
    Row(
        ...
        Icon(
            painter = painterResource(if (online) R.drawable.search else R.drawable.history),
            contentDescription = null,
            tint = if (online) Color(0xFFE50914) else Color(0xFFA3A3A3),
