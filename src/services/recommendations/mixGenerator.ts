...
export const mixGenerator = {
  getDailyMixes: async (
    ...
    try {
      ...
      const artists = [...new Set([...followedArtistNames, ...topArtists, ...favoriteArtists].filter(Boolean))];
      ...
      const mixes: DailyMix[] = genreSources.map((src, i) => {
        const result = mixResults[i];
        const raw = result.status === 'fulfilled' ? result.value : [];
        const tracks = diversify(raw, new Set(), 2).slice(0, 18);
